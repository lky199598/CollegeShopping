...
public class GoodsDao {
		...
		if (record_id != null && record_id.length() > 0) {
			...
			if (id.length >= 6)
				for (int i = id.length - 2; i > id.length - 2 - count; i--)
					list.add((Goods)JdbcReflect.Select_Information_From_ID(g, Integer.parseInt(id[i])));
	...
	{	
		...
		ResultSet	res = pre.executeQuery();
			...
			JdbcConnection.close(conn, pre, res);
	...
	{
		Connection conn = JdbcConnection.get_Connection();
		try {
			...
			pre.setInt(2, id);
			pre.executeUpdate();
			JdbcConnection.close(conn, pre, null);
	...
	public static void main(String[] args) {
		List<Object> list = JdbcReflect.Select_All_Information(new Message()); 
